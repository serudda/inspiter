...
if(Token::IsTokenValid($_SESSION['tokenId'],$_SESSION['iduser'])==1)
{
 ...
 { 
  ...
  try
  {
   ...
   if($img === false)
   {
    throw new Exception("No se puede procesar: Verifique que la URL contenga una imagen valida.");
   ...
   else
   {
    ...
    if($pasaImgSize == true)
    {
	 ...
	 $mensajeFile = $imgFile;
         ...
         $mensajeFile = 'Verifique el tamaño y tipo de imagen, recuerde que recibimos solo archivos .jpg y archivos que no sean tan pesados.';
					
    }
    else
	// Error en las dimensiones de la imagen
	$mensajeFile = 'Verifique que sea en realidad una imagen, y recuerda que las dimensiones minimas son de '.minWidth.'x'.minHeight.' pixeles.';
